...
if __name__ == '__main__':
    ...
    initial_fields_gt = np.load('./data/initial_pulse_raman_complex.npy')
    output_fields_gt = np.load('./data/output_pulse_raman_complex.npy')
    ...
    for it in range(num_iters):
        ...
        if it == 0:
             ...
             fig, ax2 = plt.subplots(1, 2,figsize=(8, 5))
             ...
             for i in range(num_modes):
                 line1, = ax2[1].plot(output_spectrum[i], '-', label=f'mode {i+1}', alpha=0.8, linewidth=2.0)
        ...
        loss = objective_function_spectrum(output_fields, target_field)
        loss.backward()
    ...
    for i in range(num_modes):
        ...
        line1, = ax_out[0].plot(domain.t, np.abs(output_fields[i]), '-', label=f'Opt mode {i+1}', alpha=0.8, linewidth=2.0)
        # Plot ground truth result with same color
        ax_out[0].plot(domain.t, np.abs(output_fields_gt[i]), '--', alpha=0.8, linewidth=1.5, color=line1.get_color(),)
    ...
    spectrum_gt = np.fft.fftshift(np.abs(np.fft.fft(np.fft.ifftshift(output_fields_gt, axes=0)))**2)

    for i in range(num_modes):
        line2, = ax_out[1].plot(spectrum_opt[i], '-', label=f'Opt mode {i+1}', alpha=0.8, linewidth=2.0)
        ax_out[1].plot(spectrum_gt[i], '--', alpha=0.8, linewidth=1.5, color=line2.get_color(),)
