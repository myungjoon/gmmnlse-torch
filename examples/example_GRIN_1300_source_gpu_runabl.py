...
if __name__ == '__main__':
    ...
    if DATA_FROM_MATLAB:
        ...
        # convert to torch on the selected device
    ...
    # --- 결과는 플로팅을 위해 CPU/NumPy로 변환 ---
    output_fields = sim.output_fields.detach().cpu().numpy()
    np.save('./data/output_fields_kerr.npy', output_fields)

    saved_fields = sim.saved_fields.detach().cpu().numpy()
    np.save('./data/saved_fields_kerr.npy', saved_fields)
    ...
    images = np.zeros((saved_fields.shape[0],saved_fields.shape[2]), dtype=np.float64)

    for i in range(saved_fields.shape[0]):
        images[i] = np.abs(np.fft.fft(np.fft.ifftshift(saved_fields[i,0])))**2
        # shift images[i] so that the peak is centered
        images[i] = np.fft.fftshift(images[i])
    plt.imshow(images, cmap='inferno', aspect='auto', origin='lower',)
