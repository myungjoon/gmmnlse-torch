...
def objective_function(output_fields, target_field):
    ...
    Args:
        ...
        target_field: torch.Tensor, shape (num_modes, Nt), complex
...
def generate_random_target_field(domain, num_modes, seed=None, device=None, dtype=torch.complex128):
    """
    ...
    coeffs_real = torch.randn(num_modes, dtype=torch.float64, device=device)
    coeffs_imag = torch.randn(num_modes, dtype=torch.float64, device=device)
    coeffs = torch.complex(coeffs_real, coeffs_imag)
    coeffs = coeffs / torch.norm(coeffs)

    print(f'Target Coeff (complex): {coeffs}')
...
if __name__ == '__main__':
    ...
    # Initial random complex coefficients for initial pulse
    coeffs_real = 0.5 + 0.5 * torch.randn(num_modes, dtype=torch.float64, device=device)
    coeffs_imag = 0.5 * torch.randn(num_modes, dtype=torch.float64, device=device)
    coeffs = torch.complex(coeffs_real, coeffs_imag)
    ...
    target_field = generate_random_target_field(domain, num_modes, device=device, dtype=torch.complex128)
    ...
    for mode_idx in range(initial_field.shape[0]):
        plt.plot(domain.t, initial_field[mode_idx].real.cpu().numpy(), label=f"Mode {mode_idx} (real)")
        plt.plot(domain.t, initial_field[mode_idx].imag.cpu().numpy(), '--', label=f"Mode {mode_idx} (imag)")
    plt.xlabel("Time (ps)")
    ...
    for mode_idx in range(target_field.shape[0]):
        plt.plot(domain.t, target_field[mode_idx].real.cpu().numpy(), label=f"Mode {mode_idx} (real)")
        plt.plot(domain.t, target_field[mode_idx].imag.cpu().numpy(), '--', label=f"Mode {mode_idx} (imag)")
    plt.xlabel("Time (ps)")
    plt.ylabel("Field Amplitude")
    plt.legend()
    plt.tight_layout()
    ...
    theta_real = torch.randn(num_modes, dtype=torch.float64, device=device, requires_grad=True)
    theta_imag = torch.randn(num_modes, dtype=torch.float64, device=device, requires_grad=True)
    theta = torch.complex(theta_real, theta_imag)
    theta.requires_grad_(True)

    # Adam does not support complex parameters directly, so we optimize real and imag separately
    optimizer = torch.optim.Adam([theta_real, theta_imag], lr=lr)
    ...
    for it in range(num_iters):
        ...
        if torch.isnan(theta_real.grad).any() or torch.isnan(theta_imag.grad).any():
            print(f"theta.grad became nan at iter {it}")
            print("theta_real:", theta_real)
            print("theta_imag:", theta_imag)
            print("theta_real.grad:", theta_real.grad)
            print("theta_imag.grad:", theta_imag.grad)
            break
