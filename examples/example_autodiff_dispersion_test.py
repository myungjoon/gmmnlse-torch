...
def objective_function_spectrum(output_fields, target_fields):
    """
    Objective function to compare the output spectrum with a target spectrum.
    Args:
        output_fields: torch.Tensor, shape (num_modes, Nt), complex
        target_fields: torch.Tensor, shape (num_modes, Nt), complex
    Returns:
        loss: torch.Tensor, scalar
    """
    # Compute output and target spectra using torch
    output_spectrum = compute_spectrum(output_fields)
    target_spectrum = compute_spectrum(target_fields)
    # Normalize both spectra for fair comparison (L2 norm)
    output_spectrum = output_spectrum / (torch.norm(output_spectrum) + 1e-12)
    target_spectrum = target_spectrum / (torch.norm(target_spectrum) + 1e-12)
    # Use L2 loss (MSE) between the output and target spectrum
    loss = torch.mean((output_spectrum - target_spectrum) ** 2)
    return loss
...
if __name__ == '__main__':
    ...
    for it in range(num_iters):
        ...
        initial_field = pulse.fields
        sim = Simulation(domain, fiber, initial_field, boundary, config)
        sim.run()
