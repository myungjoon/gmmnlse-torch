...
def objective_function_flatness(
    fields: torch.Tensor,
    ...
    eps = 1e-12
    # FFT -> spectrum power per mode (|E(f)|^2), fft along last dim
    spec = torch.fft.fftshift(
        torch.fft.fft(torch.fft.ifftshift(fields, dim=-1), dim=-1),
        dim=-1
    )
    power_per_mode = spec.abs()**2  # (num_modes, Nf)

    # Combine modes by summing intensities (orthogonal modes assumption)
    power_total = power_per_mode.sum(dim=0)  # (Nf,)

    Nf = power_total.shape[-1]
    si = 0 if start_index is None else start_index
    ei = Nf if end_index is None else end_index
    if si > ei:
        si, ei = ei, si
    # assert 0 <= si < ei <= Nf, "Invalid start/end indices for frequency range."

    # In-band region
    inband = power_total[si:ei]  # (Nb,)
    # Log domain for flatness (natural log; using log10 is fine too)
    logp = torch.log(inband + eps)
    mu = logp.mean()
    var = ((logp - mu)**2).mean()  # variance of log power = flatness loss

    loss = var

    # Optional: penalize out-of-band leakage (fraction of power outside the band)
    if leak_weight > 0:
        total_power = power_total.sum() + eps
        outband_power = power_total[:si].sum() + power_total[ei:].sum()
        leak = outband_power / total_power
        loss = loss + leak_weight * leak

    return loss

def objective_band_intensity(fields, start_index, end_index):
    output_spectrum = torch.fft.fftshift(
        torch.abs(torch.fft.fft(torch.fft.ifftshift(fields, dim=-1))) ** 2,
        dim=-1
    )
    output_spectrum = output_spectrum[:, start_index:end_index]
    loss = -torch.sum(output_spectrum)
    return loss
...
if __name__ == '__main__':
    ...
    start_wvl = 2100
    end_wvl = 2200
    start_index = np.argmin(np.abs(wavelength_nm - start_wvl))
    end_index = np.argmin(np.abs(wavelength_nm - end_wvl))

    print(f"Start wavelength: {start_wvl}")
    print(f"End wavelength: {end_wvl}")
    ...
    # make intensity sum of the design variable coeff is 1

    #random coeffs from 0 to 1
    coeffs = torch.rand(num_modes, dtype=torch.complex64, device=device, requires_grad=True)
    # coeffs = torch.ones(num_modes, dtype=torch.complex64, device=device, requires_grad=True)
    # coeffs = torch.tensor([0.9]+[0.1]*(num_modes-1), dtype=torch.complex64, device=device, requires_grad=True)
    # normalize
    ...
    # GA parameters
    population_size = 10
    num_generations = num_iters
    mutation_rate = 0.2
    elite_fraction = 0.2  # Fraction of top individuals to keep
    num_elite = max(1, int(population_size * elite_fraction))

    # Initialize population: each individual is a tensor of complex coefficients
    population = []
    for _ in range(population_size):
        coeffs_ind = torch.rand(num_modes, dtype=torch.complex64, device=device)
        population.append(coeffs_ind)
    ...
    for gen in range(num_generations):
        fitness = []
        losses_gen = []
        for idx, coeffs_ind in enumerate(population):
            # Normalize coefficients
            norm = torch.sqrt(torch.sum(torch.abs(coeffs_ind) ** 2))
            coeffs_opt = coeffs_ind / (norm + 1e-12)
            ...
            sim.run(use_cp=False)
            # print(f'computation time for forward calculation : {time.time() - start_time}')
            output_fields = sim.fields.fields

            loss = objective_band_intensity(output_fields, start_index=start_index, end_index=end_index)
            losses_gen.append(loss.item())
            fitness.append(-loss.item())  # GA maximizes fitness, so use negative loss

            # Track best
            if loss.item() < best_loss:
                best_loss = loss.item()
                best_coeffs = coeffs_ind.detach().clone()

            # Plot for the first individual of the first generation
            if gen == 0 and idx == 0:
                ...
                line1, = ax2[1].plot(wavelength_nm, spectrum_intensity_dB, '-', label=f'total', alpha=0.8, linewidth=2.0)
                ax2[1].set_xlim([1000, 2500])
                ax2[1].legend(fontsize=14)
                ax2[1].set_xlabel('Wavelength (nm)', fontsize=25)
                ax2[1].set_ylabel('Intensity (a.u.)', fontsize=20)
                plt.show()

        # Save best loss of this generation
        losses.append(torch.min(torch.tensor(losses_gen)))
        print(f"Generation {gen}: best loss = {torch.min(torch.tensor(losses_gen))}")

        # --- Selection ---
        # Get indices of top individuals (lowest loss)
        losses_gen_tensor = torch.tensor(losses_gen)
        sorted_indices = torch.argsort(losses_gen_tensor)
        elite_indices = sorted_indices[:num_elite]
        elites = [population[i].detach().clone() for i in elite_indices]

        # --- Crossover and Mutation ---
        new_population = elites.copy()
        while len(new_population) < population_size:
            # Select two parents randomly from elites
            parent_indices = torch.randint(0, len(elites), (2,))
            parent1 = elites[parent_indices[0]]
            parent2 = elites[parent_indices[1]]
            # Uniform crossover
            mask = torch.rand(num_modes, device=device) > 0.5
            child = torch.where(mask, parent1, parent2)
            # Mutation: add small random noise to real and imag parts
            if torch.rand(1).item() < mutation_rate:
                noise_real = 0.1 * torch.randn(num_modes, device=device)
                noise_imag = 0.1 * torch.randn(num_modes, device=device)
                child = child + noise_real + 1j * noise_imag
            new_population.append(child)
        # Replace population
        population = new_population[:population_size]

    # After GA, set coeffs to best found
    coeffs = best_coeffs
    ...
    # for i in range(num_modes):
    line2, = ax_out[1].plot(wavelength_nm, spectrum_intensity_dB, '-', label=f'total', alpha=0.8, linewidth=2.0)
    ax_out[1].set_xlim([1000, 2500])
