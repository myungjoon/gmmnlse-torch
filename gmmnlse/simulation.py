...
class Simulation:
    ...
    def _propagate_N_step(self, fields,):
        """Single simulation step that can be checkpointed"""
    ...
    def run(self, use_cp=False):
        ...
        for i in tqdm(range(self.domain.Nz // N), disable=is_slurm_job):
            if use_cp:
                fields = checkpoint(lambda f: self._propagate_N_step(f),
                                  fields, use_reentrant=False)
            else:
                fields = self._propagate_N_step(fields,)
