function [A1, fields_per_stage] = GMMNLSE_ss_step(A0, dt, sim, nonlin_const, mode_info, omegas, D, hrw, iter_idx)
% GMMNLSE_ss_step  Take one step according to the GMMNLSE, using the split step method
% Records 3 fields per iteration (for first 5 iterations) for debugging/comparison.
...
% iter_idx - current iteration index (1-based)
%
% Output:
% A1 - (N, num_modes) matrix with the field after the step, for each mode, in the frequency domain
% fields_per_stage - (3, N, num_modes) array with fields at 3 stages (if iter_idx <= 5), else empty

    N = size(A0, 1);
    num_modes = size(A0, 2);

    % Only record for first 5 iterations
    if nargin < 9
        iter_idx = 1;
    end
    if iter_idx <= 5
        fields_per_stage = zeros(3, N, num_modes, class(A0));
    else
        fields_per_stage = [];
    end

    % 1) Propagate through the first dispersion section
    A_t = fft(A0 .* exp(D)); % time domain

    if iter_idx <= 5
        % Save after first half-step linear (in time domain)
        fields_per_stage(1, :, :) = A_t;
    end

    % 2) Propagate through the nonlinearity with RK4
    prefactor = 1i * nonlin_const * (1 + sim.sw * omegas / (2 * pi * sim.f0));

    % Save A_t for each RK4 stage if needed
    % (For simplicity, only save after full RK4, as in PyTorch code)
    ...
    A_t_nl = A_t + 1/6 * (k1 + 2*k2 + 2*k3 + k4);

    if iter_idx <= 5
        % Save after nonlinearity (in frequency domain)
        fields_per_stage(2, :, :) = A_t_nl;
    end

    A_w_afternonlin = ifft(A_t_nl);

    % 3) propagate through the second dispersion section
    A1 = A_w_afternonlin .* exp(D);

    if iter_idx <= 5
        % Save after second half-step linear (in frequency domain)
        fields_per_stage(3, :, :) = A1;
    end

    % Optionally, save to .mat file after all iterations in main loop
    % (not here, to avoid overwriting in each call)
end
